...
export const PROGRAMS: Program[] = [
  ...
  {
    key: "mesh",
    name: "Other Nodes & Offline Sync",
    tier: 2,
    status: "live",
    route: "/nodes",
    summary: "Many small nodes instead of one server. Members trust another community's node the way you trust a bank you checked, and its board, notices, shared things and seeds show here under its name.",
    why: "A hostile government moves on the network first. A single server is a single point of seizure. What a community publishes must be replicable, signed, and checkable on another node without trusting the server it came from.",
    features: [
      "Each node known by its key, the same key that signs its ledger",
      "Trust in another node counted like vouches: verified members only, the same square-root rule",
      "Signed bundles: fetched with a signed request, sent, or carried by hand as a file",
      "Forged, edited, future-dated or untrusted bundles refused with their reason, on record",
      "A peer's ledger history held to the one it showed before; a changed history is shown",
      "Records mirrored read-only, marked withdrawn rather than deleted",
      "Planned: transfers signed by both members, so people can trade across nodes",
      "Planned: sync over a local network, mesh radio, or on a schedule",
    ],
  },
  {
    key: "apprentice",
