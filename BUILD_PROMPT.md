...
**Usable by everyone, and clear on a first visit (done, 2026-09-21).** Checked against WCAG 2.2 by reading the code, not by anyone using a screen reader, which is still owed. Found and fixed: every hint sat inside its `<label>`, so a screen reader read it as part of the field's name (now linked with `aria-describedby`, `Field` in `ui.tsx`); messages after a form were silent to screen readers (now `role="alert"`/`role="status"`), and nothing took you to the field at fault (validation failures now name their field through `failIssue`, and `ErrorFocus` focuses it); there was no way past the menu (skip link); the light "warning" color was 3.3:1 against the page and 3.6:1 on cards, too faint for small labels (now `#8f5c10`, 5.2:1); the theme editor's color text boxes and CSS box had no names; seven textareas had no guidance or only a placeholder. Every textarea now has a hint that says what to write (the bulletin hint asks for how you know, against rumor). Empty board, notices, shared things and seeds show a few made-up examples in a dashed box headed "Examples, not real posts", each marked, by names no username can hold (`src/lib/examples.ts`); they are never stored and never shown beside real posts, which keeps "only real figures" intact. The theme editor now says where a person's own colors fall below 4.5:1, in both schemes, and never blocks saving. `smoke:a11y` holds all of it; the manifest gained the capability `accessible`, with its limits. Inline graphics were then measured in a real browser across 22 pages (265 marks): the Grace sprig sat up to 3.6px off the middle of its digits (1.3px low in balances, 3.3px high in the ledger's legend) with empty margin in its drawing reading as a space before the number, and the (i) was a fixed 16px circle that sat up to 2.6px low beside headings and looked outsized beside small capitals. Both now size in em, center from the font's own metrics, and measure within half a pixel; negative amounts print a real minus sign. Still open: keeping typed input when a form comes back with an error, a text alternative for the maps beyond the lists, and a real test with assistive technology.
...
**Built (2026-09-21): taking in a node you trust, like a bank you checked.** Prerequisites (2), (4) and the information half of (3) are live; (1), member-signed transfers, is not, so nothing about money crosses yet. The node key *is* the commons key, so one pinned key checks both a peer's bundles and its ledger checkpoints. Who decides to trust a peer was the design question, since there is no admin: each verified member trusts a node the way they vouch for a person, and the node is taken in once verified trust reaches `requiredVouchesFor(verified members here)` (`peerTrustWithWork`, shown worked on the node's page). A per-member "my bank" view was considered and dropped: storage and sending are node-wide, so the decision has to be too. What travels (`buildNodeBundle`): open listings, current notices, shared things, available seeds, their authors (username, locality, public key), signed vouches for those authors, the ledger as payload hashes with a signed checkpoint, and the books as totals. Pins on records are rounded to about 1 km; balances, trades, disputes, votes and people's own pins never leave. Taken in (`ingestBundle`): refused unless signed by a trusted peer's key, with a valid checkpoint by the same key and a chain that folds to it; a later bundle's history must begin with the one held, and if it does not, the peer page says so (`historyChangedAt`), because `migrate:grace-cents` also re-derives chains and only people there can say which it was. Records are mirrored read-only, claim-then-apply in one transaction, marked `goneAt` rather than deleted, and every bundle from a known peer is logged in `PeerIngest`. Three ways to move one: fetch (signed request headers, only the new ledger hashes), send (POST to the peer's `/api/federation/ingest`), or a file carried by hand. The fetch refuses private addresses by name and by DNS answer; it does not pin the resolved address (noted in the manifest). API and formats: `docs/federation.md`. Held by `smoke:federation`. Next: member-signed transfers, then crediting a transfer signed on another node.
