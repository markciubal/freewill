...
**Map looks and depth (done).** The map is themed per person: presets (Cypherpunk by default: near-black ground, glowing cyan roads; Paper, Blueprint, Phosphor) or any color, with a glow width and a picture-tile filter for the raster fallback, previewed live in the theme editor. The UI gained depth (an `elevation` token drives tinted shadows, gradient buttons, a glass header); `0px` returns it to flat.
