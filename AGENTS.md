...
- **ID.me attestation (exception to "no OAuth").** Attestation only, never login, entirely behind IDME_ENABLED (default off). Store only `humanVerifiedAt` + `idmeHash` (HMAC of subject, app-level uniqueness in the callback). Its only mechanical effect is one extra counted vouch in standing; it must never gate credit, disputes, proposing, or voting.
